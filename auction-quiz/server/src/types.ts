...
export interface TeamRegistration {
  ...
  email: string;
  deviceId: string;
  teamName?: string; // Disallowed/ignored by backend; automatically assigned from pool
}
...
export interface ClientEvents {
  ...
  "admin:get_images": (
    cb: (res: { success: boolean; images?: { name: string; path: string; folder: string }[]; error?: string }) => void
  ) => void;
